...
import com.catlab.ping.ui.HeartRateSettingsActivity
...
class MainActivity : AppCompatActivity() {
    ...
    private lateinit var switchHeartrate: MaterialSwitch
    private lateinit var tvHeartrateStatus: TextView
    private lateinit var btnHeartrateSettings: MaterialButton
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        switchHeartrate = findViewById(R.id.switch_heartrate)
        tvHeartrateStatus = findViewById(R.id.tv_heartrate_status)
        btnHeartrateSettings = findViewById(R.id.btn_heartrate_settings)
        ...
        switchHeartrate.isChecked = prefs.getBoolean("heartrate_enabled", false)

        updateLocationStatus(switchLocation.isChecked)
        updateScreenshotStatus(switchScreenshot.isChecked)
        updateHeartrateStatus(switchHeartrate.isChecked)
        ...
        switchHeartrate.setOnCheckedChangeListener { _, isChecked ->
            prefs.edit().putBoolean("heartrate_enabled", isChecked).apply()
            if (isChecked) {
                val serverUrl = prefs.getString("heartrate_server", "") ?: ""
                if (serverUrl.isBlank()) {
                    Toast.makeText(this, "请前往设置确认心率接收服务端地址", Toast.LENGTH_SHORT).show()
                }
                
                // 如果启用了BLE直连模式，启动相应的服务
                if (prefs.getBoolean("ble_direct_enabled", false)) {
                    val intent = Intent(this, com.catlab.ping.service.BleHeartRateService::class.java)
                ...
                updateHeartrateStatus(true)
                Toast.makeText(this, "💖 心率监听已开启", Toast.LENGTH_SHORT).show()
            } else {
                stopService(Intent(this, com.catlab.ping.service.BleHeartRateService::class.java))
                updateHeartrateStatus(false)
                Toast.makeText(this, "💖 心率监听已关闭", Toast.LENGTH_SHORT).show()
            }
        }
        ...
        btnHeartrateSettings.setOnClickListener {
            startActivity(Intent(this, HeartRateSettingsActivity::class.java))
        }
    }
    ...
    private fun updateHeartrateStatus(enabled: Boolean) {
        if (enabled) {
            tvHeartrateStatus.text = "● 监听中"
            tvHeartrateStatus.setTextColor(0xFF4CAF50.toInt())
        } else {
            tvHeartrateStatus.text = "○ 已关闭"
            tvHeartrateStatus.setTextColor(0xFF9E9E9E.toInt())
        }
    }
}
